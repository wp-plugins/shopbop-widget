...
class CoreWidgetAdmin
{
    ...
    public function addSettingsScript()
    {
        ...
        wp_enqueue_style(constant($this->widgetPrefix.'PUBLIC_WIDGET_NAME_SLUG') . '-Admin-css', constant($this->widgetPrefix.'PLUGIN_DIR_URL') . 'css/admin_widget.css?where=settings&modified=20140201');
        if(version_compare($wp_version,"3.3","<"))
        {
            wp_enqueue_script(constant($this->widgetPrefix.'PUBLIC_WIDGET_NAME_SLUG') . '-Admin-js', constant($this->widgetPrefix.'PLUGIN_DIR_URL') . 'js/admin_widget.js?where=settings&modified=20140201');
...
//         Add Appearence sections
        ...
        add_settings_section('core_widget_affiliate_information_section', '', array($this, 'widgetAffiliateInformationCb'), 'core_widget_affiliate_information_section');
        ...
        add_settings_field(
            constant($this->widgetPrefix.'PUBLIC_WIDGET_NAME_SLUG').
            'widget_pane_to_open', 'Widget pane to open',
            array(
                 $this,
                 'widgetPaneToOpen',
            ),
        ...
        add_settings_field(
            constant($this->widgetPrefix.'PUBLIC_WIDGET_NAME_SLUG').
            'widget_affiliate_information', 'Commission Junction Publisher ID',
            array(
                 $this,
                 'widgetAffiliateInformation',
            ),
            'core_widget_affiliate_information_section',
            'core_widget_affiliate_information_section'
        );
    ...
    /**
     * Widget Affiliate Information call back method.
     *
     * @return void
     */
    public function widgetAffiliateInformationCb()
    {
        //no-op
    }
    ...
    public function widgetOptionsValidationCb($input = null)
    {
        ...
        if(!array_key_exists('registered', $widgetWsOptions) || $widgetWsOptions['registered'] === false || !array_key_exists('widgetId', $widgetWsOptions) || is_null($widgetWsOptions['widgetId']))
        {
            ...
            $validInput['widget_pane_to_open']          = constant($this->widgetPrefix . 'WIDGET_DEFAULT_PANE_TO_OPEN');
            $validInput['widget_affiliate_information'] = constant($this->widgetPrefix . 'WIDGET_DEFAULT_AFFILIATE_INFO');
        ...
        if (array_key_exists('widget_affiliate_information', $widgetOptions))
        {
            if ($widgetOptions['widget_affiliate_information'] == 'nothanks')
            {
                $validInput['widget_affiliate_information'] = $widgetOptions['widget_affiliate_information'];
            }
            elseif ($widgetOptions['widget_affiliate_information'] == 'cjAffiliateSelected')
            {
                (isset($widgetOptions['cjPublisherID'])) ? $validInput['widget_affiliate_id'] = trim($widgetOptions['cjPublisherID']) : $validInput['widget_affiliate_information'] = "";
                $validInput['widget_affiliate_information'] = $widgetOptions['widget_affiliate_information'];

            }
        }

        if(array_key_exists('widget_pane_to_open', $widgetOptions))
            $validInput['widget_pane_to_open'] = $widgetOptions['widget_pane_to_open'];
    ...
    {
        $pane   = (array_key_exists('widget_pane_to_open', $this->options)) ? $this->options['widget_pane_to_open'] : constant($this->widgetPrefix . 'WIDGET_DEFAULT_PANE_TO_OPEN');
        $params = array(
            'pane'       => $pane,
        );
        $widgetView = new CoreWidgetBase($this->widgetPrefix);
     ...
     *
     */
    public function widgetAffiliateInformation()
    {
        $params = array();
        $aff    = (array_key_exists('widget_affiliate_information', $this->options)) ? $this->options['widget_affiliate_information'] : constant($this->widgetPrefix . 'WIDGET_DEFAULT_AFFILIATE_INFO');
        $affid  = (array_key_exists('widget_affiliate_id', $this->options)) ? $this->options['widget_affiliate_id'] : "";
        $params = array(
            'aff'   => $aff,
            'affid' => $affid,
        );
        $widgetView = new CoreWidgetBase($this->widgetPrefix);
        $widgetView->loadView('admin/widget-affiliate-information', $params);
        return;
    }
