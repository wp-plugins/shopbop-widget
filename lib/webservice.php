...
class CoreWebservice
{
	 ...
	 * @return array
	 */
	public function getPaneContent($postId, $paneId, $path, $queries = null, $lang = null, $wpCron = null)
	{
        if(is_null($lang))
    ...
    {
        $time           = (int)gmdate('U');
        $gaApiResponse = $this->getItemFromApi('/google-analytics-code-control/');
        $gaOption      = array(
            'gaLastUpdatedTime' => $time,
            'gaStatus'          => filter_var($gaApiResponse['body']['ga-code'], FILTER_VALIDATE_BOOLEAN),
        );
     ...
     * @return mixed
     */
    public function getItemFromAnywhereWithCacheUpdate($postId, $path, $url, $type, $returnKey = null, $wpCron = null)
    {
